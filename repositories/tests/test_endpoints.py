...
class EndpointsTests(APITestCase):
    ...
    def test_list_commits_filtered_author(self):
        response1 = self.client.get('/api/commits/?author=renato', format='json')
        self.assertEqual(response1.data.get("count"), 4)
        response2 = self.client.get('/api/commits/?author=felipe', format='json')
        self.assertEqual(response2.data.get("count"), 5)

    def test_list_commits_filtered_repo(self):
        response1 = self.client.get('/api/commits/?repository=repo1', format='json')
        self.assertEqual(response1.data.get("count"), 3)
        response2 = self.client.get('/api/commits/?repository=repo2', format='json')
        self.assertEqual(response2.data.get("count"), 3)
        response3 = self.client.get('/api/commits/?repository=repo3', format='json')
        self.assertEqual(response3.data.get("count"), 3)

    def test_list_commits_filtered_author_and_repo(self):
        response1 = self.client.get('/api/commits/?author=renato&repository=repo1', format='json')
        self.assertEqual(response1.data.get("count"), 1)
        response2 = self.client.get('/api/commits/?author=felipe&repository=repo1', format='json')
        self.assertEqual(response2.data.get("count"), 2)
        response3 = self.client.get('/api/commits/?author=renato&repository=repo2', format='json')
        self.assertEqual(response3.data.get("count"), 3)
        response4 = self.client.get('/api/commits/?author=felipe&repository=repo2', format='json')
        self.assertEqual(response4.data.get("count"), 0)
        response5 = self.client.get('/api/commits/?author=renato&repository=repo3', format='json')
        self.assertEqual(response5.data.get("count"), 0)
        response6 = self.client.get('/api/commits/?author=felipe&repository=repo3', format='json')
